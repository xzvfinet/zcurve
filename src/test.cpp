...
bool higherPrecisionTest(bool print) {
	...
	for (int i = 0; i < NUM_RANDOM_DATA; ++i)
	...
	*/
	...
	for (int i = 0; i < 3; ++i) {
		floored[i] = uint64_t(boost::multiprecision::cpp_dec_float_50(arr[0][i]) *  boost::multiprecision::cpp_dec_float_50(maxCells));
		std::cout << floored[i] << " ";
	}
	std::cout << std::endl;
	...
	for (int i = 0; i < arr.size(); ++i) {
		if (arr1[i].second != arr2[i]) {
			...
			std::cout << arr1[i].second << "\t" << zcurve.getMortonKey(arr1[i].second)
				<< "\n is different from " << arr2[i] << "\t" << zcurve.getMortonKey(arr2[i]) << std::endl;
			break;
		}
	}
	return result;
